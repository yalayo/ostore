   ...
   [:main {:role "main"}
    [:section.jumbotron.text-center
     [:div.container
      ...
      [:p
       [:button {:class "btn btn-primary my-2" :on-click #(dispatch [:set-showing (keyword "signup")])} "Sign up now"]
       [:button {:class "btn btn-secondary my-2" :on-click #(dispatch [:set-showing (keyword "login")])} "Login"]]]]])