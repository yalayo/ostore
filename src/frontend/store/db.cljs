...
(def ls-key "products-reframe")                         ;; localstore key

(defn products->local-store
  "Puts products into localStorage"
  [products]
  (.setItem js/localStorage ls-key (str products)))     ;; sorted-map written as an EDN map
...
(re-frame/reg-cofx
 ...
 (fn [cofx _]
   ;; put the localstore products into the coeffect under :local-store-products
   (assoc cofx :local-store-products
          ;; read in products from localstore, and process into a sorted map
          (into (sorted-map)
