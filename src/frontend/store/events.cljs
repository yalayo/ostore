(ns store.events
  (:require
    [store.db    :refer [default-db products->local-store]]
...
(def check-spec-interceptor (after (partial check-and-throw :store.db/db)))
...
(def ->local-store (after products->local-store))
