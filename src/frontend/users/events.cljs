...
(defn get-user-pool []
  (new CognitoUserPool
    (clj->js
     {:UserPoolId (get-in @users-state [:cognito :userPoolId])
      :ClientId (get-in @users-state [:cognito :userPoolClientID])})))
    ...
    (when (some? u)
          (go
           (let [field-ch (chan)
             ...
             (swap! users-state assoc-in [:cognito-user :email] email)
             (get-authtoken
              (fn [err t]
             ...
             (let [[token ch] (alts! [field-ch err-ch (timeout 10000)])]
               (cond
                (= ch field-ch) (swap! users-state assoc-in [:cognito-user :jwt-token] token)
             ...
             (let [[session ch] (alts! [field-ch err-ch (timeout 10000)])]
               (cond
                (= ch field-ch) (swap! users-state assoc-in [:cognito-user :valid-session]
                                       session)
...
(defn configure-aws-cognito []
  (set! (->  AWSCognito .-config .-region) (get-in @users-state [:cognito :region])))
...
(defn register-new-user []
  (let [password (get-in @users-state [:auth-form :register :password])
        email (get-in @users-state [:auth-form :register :email])
...
(defn verify-user []
  (let [email (get-in @users-state [:auth-form :verify :email])
        code (get-in @users-state [:auth-form :verify :code])
...
(defn signin-user []
  (let [email (get-in @users-state [:auth-form :signin :email])
        password (get-in @users-state [:auth-form :signin :password])
...
(defn trigger-password-recovery []
  (let [email (get-in @users-state [:auth-form :forgot :email])
        user (create-cognito-user email)]
...
(defn confirm-password-recovery []
  (let [email (get-in @users-state [:auth-form :forgot :email])
        code (get-in @users-state [:auth-form :forgot :code])
        new-password (get-in @users-state [:auth-form :forgot :new-password])
        user (create-cognito-user email)]
...
(defn subscribe-form
  [& path]
  (reagent/cursor users-state (into [] (concat [:auth-form] path) )))

(defn reset-forms []
  (swap! users-state assoc :auth-form
         {:register {}
          :signin {}
          :verify {}
          :forgot {}}))
...
(defn reset-cognito-user [u]
  (swap! users-state assoc :cognito-user u))

(defn conj-form-flash [form m]
  (swap! users-state update-in [:auth-form form :flash]
         conj m))

(defn clear-form-flash [form]
  (swap! users-state assoc-in [:auth-form form :flash] []))

(defn clear-signin-password []
  (swap! users-state assoc-in [:auth-form :signin :password] nil))

(defn clear-signin-email []
  (swap! users-state assoc-in [:auth-form :signin :email] nil))
